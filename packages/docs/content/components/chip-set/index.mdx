...
Set `removable` to add a remove button to every chip. The chips are controlled by your data, so drop the chip in the `onRemove` callback. When a chip is removed, focus moves to a neighboring chip.
